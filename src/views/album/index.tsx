        ...
        <div className='more'>
            ...
            <div className="index-row">
                <Row gutter={[44, 24]}>
                    {data.map((item, index) => {
                        return (
                            <Col onClick={()=>{navigate('/album/?id='+artist?.hotSongs[index].al.id)}} key={index} span={4} >
                                <CommonCard
                                    imgSrc={(artist?.hotSongs[index].al.picUrl !==undefined)?(artist.hotSongs[index].al.picUrl):''}
                                    title={artist?.hotSongs[index].al.name}
                                    shape="round"
                                    textPostion="left"
                                    goToAlbum={()=>{navigate('/album/?id='+artist?.hotSongs[index].al.id)}}
                                />
                            </Col>
                        );
                    })}
        ...
        <div className='album'>
            <div className='album-msg'>
                ...
                <div className='album-detail'>
                    ...
                    <p>Singer by <u style={{cursor:'pointer'}} onClick={()=>{navigate('/artist/?id='+artistId)}}>{album?.album.artist.name}</u></p>
                    <p>{date}.{album?.album.size}首歌,{Math.floor(dt/1000/60)}分钟</p>
                    <p style={{width:500, height:50, overflow:'hidden',textOverflow:'ellipsis'}}>{album?.album.description}</p>
                    <Space size='large'>
                ...
                {(album !== undefined)? (
                    <More artist={album.album.artist.id} />
                ):(<div className='more'>More by</div>)}
            </div>
        </div>
    )
}
